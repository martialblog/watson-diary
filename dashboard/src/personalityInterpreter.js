class personalityInterpreter {
  ...
  // Provide this method with an object: { facet: percentile, [...] }. For exampl: { need_challenge: 0.92928, facet_cooperation: 0.38433 }
  // Returns an array of strings, describing the most prominent traids of the author in connection to openness.
  interpretNeeds(needs) {
    ...
    if (need_excitement > highValue) {
      result.push('You want to get out there and live life, have upbeat emotions, and want to have fun.');
    }

    if (need_harmony > highValue) {
      result.push('You appreciate other people, their viewpoints, and their feelings.');
    }

    if (need_curiosity > highValue) {
      result.push('You have a desire to discover, find out, and grow.');
    }

    if (need_ideal > highValue) {
      result.push('You desire perfection and a sense of community.');
    }

    if (need_closeness > highValue) {
      result.push('You relish being connected to family and setting up a home.');
    }

    if (need_self_expression > highValue) {
      result.push('You enjoy discovering and asserting their own identities.');
    }

    if (need_liberty > highValue) {
      result.push('You have a desire for fashion and new things, as well as the need for escape.');
    }

    if (need_love > highValue) {
      result.push('You enjoy social contact, whether one-to-one or one-to-many. Any brand that is involved in bringing people together taps this need.');
    }

    if (need_practicality > highValue) {
      result.push('You have a desire to get the job done, a desire for skill and efficiency, which can include physical expression and experience.');
    }

    if (need_stability > highValue) {
      result.push('You seek equivalence in the physical world. They favor the sensible, the tried and tested.');
    }

    if (need_challenge > highValue) {
      result.push('You have an urge to achieve, to succeed, and to take on challenges.');
    }

    if (need_structure > highValue) {
      result.push('You exhibit groundedness and a desire to hold things together. They need things to be well organized and under control.');
    }
